...
def train(train_loader, model, criterion, optimizer, args, logger,warmup_scheduler=None):
    ...
    if dist.get_rank() == 0:
        ...
        metric_func = voc_mAP
        mAP, aps = metric_func([os.path.join(args.output, _filename) for _filename in filenamelist], args.num_labels,
                               return_each=True)
...
def validate(val_loader, model, criterion, args, logger, best_map=None ):
    ...
    with torch.no_grad():
        ...
        if dist.get_rank() == 0:
            ...
            metric_func = voc_mAP                
            mAP, aps = metric_func([os.path.join(args.output, _filename) for _filename in filenamelist], args.num_labels, return_each=True)
