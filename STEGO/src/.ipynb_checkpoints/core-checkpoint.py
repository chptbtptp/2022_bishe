...
# modify
class HeartTUSeg(Dataset):
    def __init__(self, root, image_set, transform, target_transform, coarse_labels):
        super(HeartTUSeg, self).__init__()
        self.split = image_set
        self.root = os.path.join(root, "heartTU_vein")
        self.transform = transform
        self.target_transform = target_transform
        self.files = json.loads(open(os.path.join(self.root,'heartTU_annotation.json'), 'r').read())[self.split]
        if '.ipynb_checkpoints' in self.files:
    ...
    def __getitem__(self, index):
        image_id = self.files[index]
        image_id = os.path.join(self.root,image_id)
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        if dataset_name == "potsdam":
            ...
            extra_args = dict(coarse_labels=True)
        #modify
        elif dataset_name == "heartTU":
            self.n_classes = 2
            dataset_class = HeartTUSeg
            extra_args = dict(coarse_labels=True)
        ...
        feature_cache_file = join(pytorch_data_dir, "nns", "nns_vein_{}_{}.npz".format(
            image_set, crop_type))
